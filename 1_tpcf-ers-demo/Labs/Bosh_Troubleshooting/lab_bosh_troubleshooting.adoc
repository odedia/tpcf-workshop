...
+
. Repeat killing the `rep` process and view the effects in `Elastic Runtime` tile `Status` tab within Ops Manager.
+
image::rep-down.png[]
...
Learn more about using BOSH to troubleshoot a tPCF installation.  https://blog.pivotal.io/labs/labs/troubleshooting-bosh-releasesdeployments[This blog] is a great starting point.
