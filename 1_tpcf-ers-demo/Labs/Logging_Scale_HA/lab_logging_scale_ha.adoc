...
[abstract]
--
How tPCF facilitates application management, including: access application 
...
== High Availability
Tanzu Platform for Cloud Foundry has https://content.pivotal.io/blog/the-four-levels-of-ha-in-pivotal-cf[4 levels of HA] (High Availability) that keep your applications and the underlying platform running. In this section, we will demonstrate one of them. Failed application instances will be recovered.
+
...
$ cf app attendees
----
Sample output below (notice the requested state vs actual state). In this case, Tanzu Platform for Cloud Foundry had already detected the failure and is starting a new instance.
----
